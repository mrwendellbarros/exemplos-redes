...
int main()
{
	...
	/* Cria sockect TCP ----------------------------------------------------- */
	if ((sockAcesso = socket(AF_INET, SOCK_STREAM, 0)) < 0)
		erro("Erro no socket");
	...
	for (;;)
	{
		...
		{
			/* Processo Pai */
			printf("Ligacao estabelecida com cliente %s:%d. Processo %d...\n",
						 inet_ntoa(cli_addr.sin_addr), ntohs(cli_addr.sin_port), pid);
		}
		close(sockConexao);
	} /* End for */
	close(sockAcesso);
